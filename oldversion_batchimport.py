...
class IMPORT_OT_psk(bpy.types.Operator, ImportProps):
    ...
    def execute(self, context):
        ...
        if not self.directory:
            ...
            for _, fileListElement in enumerate([f for f in scandir(self.directory) if isfile(f.path) and f.name.lower().endswith(('.psk','.pskx'))]):
...
class IMPORT_OT_psa(bpy.types.Operator, ImportProps):
    ...
    def execute(self, context):
        ...
        for _, fileListElement in enumerate([f for f in scandir(self.directory) if isfile(f.path) and f.name.lower().endswith('.psa')]):
            fpath = self.directory + fileListElement.name
            psaimport(
                fpath,
                context = context,
